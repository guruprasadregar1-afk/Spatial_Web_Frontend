...
export const CardNodeRenderer: React.FC<RendererProps> = ({
  ...
  const { matrices, count } = useMemo(() => {
    const rx = slabWidth / 2 - 0.15;
    const rz = slabDepth / 2 - 0.15;
  ...
  return (
    ...
    >
      {/* Integrated Styled Tech Badge on Top of Slab */}
      <Html position={[0, 0.4, 0]} center distanceFactor={22} zIndexRange={[10, 0]}>
        <div
          className={`glass-panel px-3 py-1.5 rounded-xl border transition-all shadow-2xl pointer-events-none whitespace-nowrap flex flex-col gap-0.5 min-w-[130px] items-center ${
            isSelected
